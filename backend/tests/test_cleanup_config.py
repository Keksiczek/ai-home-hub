...
def test_get_cleanup_config_structure(client):
    ...
    assert resp.status_code == 200
    data = resp.json()
    for key in ("enabled", "interval_hours", "session_retention_days", "artifact_retention_days", "vacuum_enabled"):
...
def test_patch_cleanup_config_saves_values(client):
    ...
    resp = client.patch("/api/settings/cleanup", json={
        "interval_hours": 12,
...
def test_cleanup_service_loads_config_dynamically():
    ...
    with patch("app.services.cleanup_service._load_cleanup_config", return_value=mock_cfg):
        # run_now should use the mocked config
        with patch.object(svc, "_cleanup_old_sessions", return_value=0) as mock_sessions, \
             patch.object(svc, "_archive_old_kb_data", return_value=0) as mock_archive, \
             patch.object(svc, "_vacuum_databases") as mock_vacuum:
...
def test_cleanup_service_skips_when_disabled():
    ...
    with patch("app.services.cleanup_service._load_cleanup_config", return_value=disabled_cfg):
...
def test_cleanup_service_runs_when_enabled():
    ...
    with patch("app.services.cleanup_service._load_cleanup_config", return_value=enabled_cfg):
        with patch.object(svc, "_do_cleanup") as mock_do:
            result = svc.run_now()
