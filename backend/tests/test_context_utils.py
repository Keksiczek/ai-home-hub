...
def _make_mock_memory_svc(has_data=True):
    ...
    if has_data:
        ...
        svc.search_memory = AsyncMock(return_value=[
            MemoryRecord(
                id="mem_aaa", text="User prefers Czech", tags=["preference"],
                source="ui", importance=8, timestamp="2025-01-01T00:00:00+00:00",
                distance=0.15,
            ),
        ])
    else:
...
async def test_get_kb_context_returns_context():
    ...
    with patch("app.utils.context_utils.get_settings_service", return_value=mock_settings), \
         patch("app.services.vector_store_service.get_vector_store_service", return_value=mock_vs), \
         patch("app.services.embeddings_service.get_embeddings_service", return_value=mock_emb):
...
async def test_enrich_message_adds_memory_and_kb_flags():
    ...
    with patch("app.utils.context_helpers.get_kb_context", new_callable=AsyncMock, return_value="kb stuff"), \
